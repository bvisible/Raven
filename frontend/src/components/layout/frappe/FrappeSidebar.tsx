 ...
 * fed by the mini-boot returned by `raven.api.boot.get_navbar_boot()`.
 *
...
export const FrappeSidebar: FC = () => {
	...
	//   1. The app whose name matches "raven" (we're inside /raven).
	...
	const currentApp: AppEntry = useMemo(() => {
		...
		const ravenApp = apps.find((a) => a.app_name === 'raven')
		if (ravenApp) return ravenApp
