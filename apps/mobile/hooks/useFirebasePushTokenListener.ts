...
const useFirebasePushTokenListener = () => {
    ...
    useEffect(() => {
        ...
        //// The tracing left over from that pass ran on every app start; behind __DEV__ since
        //// 2026-09-04.
        if (__DEV__) console.log('[PushToken] Hook triggered, siteInfo:', siteInfo?.sitename, 'callMade:', callMade.current)
        ...
        if (siteInfo) {
            //// Neoffice - push registration tracing (b30ad1930, 2026-01-05 "fix(mobile): Android push notifications and notification toggle"). __DEV__ only (2026-09-04).
            if (__DEV__) console.log('[PushToken] Requesting permission...')
            messaging.requestPermission().then(async (authorizationStatus) => {
                //// Neoffice - push registration tracing (b30ad1930, 2026-01-05 "fix(mobile): Android push notifications and notification toggle"). __DEV__ only (2026-09-04).
                if (__DEV__) console.log('[PushToken] Permission status:', authorizationStatus)
                if (authorizationStatus === AuthorizationStatus.AUTHORIZED) {
                    ...
                    //// The tracing printed the first 30 characters of the FCM token; that line is gone
                    //// (2026-09-04). A push token is a credential for sending to this device: it is
                    //// never logged, not even truncated, not even in a development build.
                    try {
                        const token = await messaging.getToken()
                        if (__DEV__) console.log('[PushToken] FCM token received')
                        ...
                        if (__DEV__) console.log('[PushToken] Mobile Device register response:', response)
                        ...
                        try {
                            ...
                            //// Neoffice - __DEV__ only (2026-09-04), see above.
                            if (__DEV__) console.log('[PushToken] Raven subscribe done')
                        } catch (ravenError) {
                            //// Neoffice - __DEV__ only (2026-09-04), see above.
                            if (__DEV__) console.log('[PushToken] Raven subscribe skipped:', ravenError)
                        }
                    } catch (error) {
                    ...
                    //// Neoffice - __DEV__ only (2026-09-04), see above.
                    if (__DEV__) console.log('[PushToken] Permission NOT authorized')
                }
        ...
        //// Neoffice - push registration tracing (b30ad1930, 2026-01-05 "fix(mobile): Android push notifications and notification toggle"). __DEV__ only (2026-09-04).
            ...
            if (__DEV__) console.log('[PushToken] No siteInfo, skipping')
        }
